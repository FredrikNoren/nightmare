  ...
  if (typeof arg === 'number') {
    ...
    if(arg < this.options.waitTimeout){
      ...
      waitms(this.options.waitTimeout, function(){
        done(new Error('.wait(' + args.map(a => '' + a).join(', ') + ') timed out after '+this.options.waitTimeout+'msec'));
      }.bind(this));
    }
  }
...
function waitfn() {
  ...
  var timeoutTimer = setTimeout(function(){
    ...
    done(new Error(`.wait(${args.map(a => '' + a).join(', ')}) timed out after ${self.options.waitTimeout}msec`));
